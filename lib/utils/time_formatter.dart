...
/// ISO 8601 형식의 시간 문자열을 한국 시간으로 변환한 뒤, "오전 9:00" 형식으로 리턴
String formatKoreanTimeFromISO(String? raw) {
  if (raw == null) return '시간 없음';
  try {
    final dt = DateTime.parse(raw).toLocal();
    final hour = dt.hour % 12 == 0 ? 12 : dt.hour % 12;
...
/// ISO 8601 형식의 시간 문자열을 한국 시간으로 변환한 뒤, "YYYY.MM.DD 오전/오후 HH:MM" 형식으로 리턴
String formatKoreanDateTimeFromISO(String? raw) {
  if (raw == null) return '시간 없음';
  try {
    final dt = DateTime.parse(raw).toLocal();
    final year = dt.year.toString();
